...
class Anggota extends Component {
  ...
  handleOpen = args => {
    this.setState({ open: true, dataUserSelected: args });
  };

  handleClose = () => {
    this.setState({ open: false, dataUserSelected: {} });
  };

  render() {
    return (
      <Grid style={{ display: 'flex' }}>
        <Paper style={{ padding: 30, backgroundColor: 'white', height: '100vh' }}>
        ...
        <Grid style={{ padding: 30, width: '100%' }}>
          <Paper style={{ width: '95%', padding: 20 }}>
            ...
            <Table>
              ...
              <TableBody>
                {
                  this.state.dataAnggotaSearch.slice(this.state.page * this.state.rowsPerPage, this.state.page * this.state.rowsPerPage + this.state.rowsPerPage).map((el, index) => (
                    <CardAnggota data={el} key={index} detailAnggota={this.handleOpen}/>
                  ))
                }
              </TableBody>
            </Table>
        ...
        <ModalDetailAnggota open={this.state.open} data={this.state.dataUserSelected} handleClose={this.handleClose}/>
      </Grid>
    )
  }
}
